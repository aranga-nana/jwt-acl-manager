...
function generateToken(secret, payload, option) {
    var optionCopy = deepcopy(option);
    //add current time as issue at time if not given
    if (!optionCopy.iat){
        //optionCopy.iat = new Date().getTime();
    }
    var deferred = q.defer();
    jwt.sign(payload, secret, optionCopy, function (err, token) {
        //console.log(data);
        if (err) {
            deferred.reject(err);
        }
